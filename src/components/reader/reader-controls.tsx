...
export function ReaderControls({
  ...
  return (
    <>
      ...
      {/* Bottom toolbar — reading options */}
      {children && (
        <div
          className={cn(
            "absolute bottom-12 left-0 right-0 z-50 transition-opacity duration-300",
            visible ? "opacity-100" : "opacity-0 pointer-events-none",
          )}
        >
          <div className="mx-auto max-w-md px-4">
            <div className="flex items-center justify-center gap-3 rounded-2xl bg-black/80 backdrop-blur-sm px-4 py-2.5">
              {children}
            </div>
          </div>
        </div>
